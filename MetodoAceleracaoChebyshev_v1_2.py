'''
File: MetodoAceleracaoChebyshev_v1_2.py
-
...
w2=(2*u*T1u)/(polche(u,2))
print "w2:\n"+str(w2)

y2=y0+np.dot(w2,(y1-y0+i*z1))
print "y2:\n"+str(y2)

w3=(2*u*polche(2,2))/polche(2,3)
print "w3:\n"+str(w3)

z2=np.matrix([[(la.inv(P)*b-np.dot(np.dot(la.inv(P),A),y2))[0,0]],[(la.inv(P)*b-np.dot(np.dot(la.inv(P),A),y2))[1,1]]])
print "z2:\n"+str(z2)

y3=y1+np.dot(w3,y2-y1+i*z2)
print "y3:\n"+str(y3)
