''''
File: sistemaEDO_v1.py
    -
Author: AnaSapata, ana_sapata@sapo.pt
Date:10|Dez|2015
Version:1.0
'''

import numpy as np
import matplotlib.pyplot as plt

def fx(yk):
    return -np.pi*yk

def fy(xk):
    return np.pi*xk

def EulerProg(x0,y0,h,t0,T):
    y=[y0]
    x=[x0]
    n=int((T-t0)/h)
    for i in range(n-1):
        yk=y[i]+h*fy(x[i])
        y.append(yk)
        xk=x[i]+h*fx(y[i])
        x.append(xk)
    return y,x

x0=1
y0=0
h=0.1/8
t0=0
T=0.5
n=int((T-t0)/h)
y,x=EulerProg(x0,y0,h,t0,T)
t=[]
for i in range(n):
    t.append(round(t0+i*h,2))

def exatax(t):
    return np.cos(np.pi*t)

def exatay(t):
    return np.sin(np.pi*t)

ey=[]
ex=[]
for i in range(n):
    ey.append(exatay(t[i]))
    ex.append(exatax(t[i]))
print "y pelo metodo:\n",y
print "exata de y:\n",ey
print "x pelo metodo:\n",x
print "exata de x:\n",ex

soly=y
solx=x
N=int((T-t0)/h)

r=np.linspace(t0,T,num=N)
sx=ex
sy=ey

#erro=s-sol

ax=plt.axes()
ax.grid(True)

plt.rc('text',usetex=False)
plt.rc('font',family='serif')

#plt.figure(1)
plt.plot(r,soly,'kx',label='Approximated solution(y)')
plt.plot(r,sy,'r', label='Exact solution(y)')

#plt.figure(2)
plt.plot(r,solx,'ko',label='Approximated solution(x)')
plt.plot(r,sx,'b', label='Exact solution(x)')

#plt.plot(r,erro,'k_', label='Error')

plt.axis([0.,0.5,-0.5,1.5])

plt.xlabel(r'time(s)',fontsize=16)

plt.title(r"Euler's method", fontsize=20,color='blue',y=1.08)
plt.subplots_adjust(top=0.8)

legend=plt.legend(loc='upper center',prop={'size':14}, shadow=True, fontsize='x-large')
legend.get_frame().set_facecolor('#C0F9E2')

plt.show()
