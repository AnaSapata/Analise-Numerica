# -*- coding: utf-8 -*-
''''
File: sistemaEDO_v3.py
    - Resolução de sistema de EDO's com o RK4/2
...
def RK42(x0,y0,h,t0,T):
    ...
    for i in range(n):
        ...
        F1y=fy(t,x[i],y[i])
        F2y=fy(t+0.25*h,x[i]+0.25*h*F1y,y[i])
        F3y=fy(t+0.5*h,x[i]+0.5*h*F2y,y[i])
        F4y=fy(t+h,x[i]+h*F1y-2*h*F2y+2*h*F3y,y[i])
        yk=y[i]+(h/6)*(F1y+4*F3y+F4y)
        y.append(yk)
        #para calcular o xk+1
        F1x=fx(t,x[i],y[i])
        F2x=fx(t+0.25*h,x[i],y[i]+0.25*h*F1x)
        F3x=fx(t+0.5*h,x[i],y[i]+0.5*h*F2x)
        F4x=fx(t+h,x[i],y[i]+h*F1x-2*h*F2x+2*h*F3x)
        xk=x[i]+(h/6)*(F1x+4*F3x+F4x)
        x.append(xk)
        t+=h
    return y,x
